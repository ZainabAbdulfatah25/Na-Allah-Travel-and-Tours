...
function Payment() {
  ...
  useEffect(() => {
    ...
    if (qIndex !== -1) {
      const searchParams = new URLSearchParams(hash.substring(qIndex + 1));
      const pkg = searchParams.get('pkg');
      if (pkg) setFormData(prev => ({...prev, packageDetails: decodeURIComponent(pkg)}));
      
      const inquiry = searchParams.get('inquiry') === 'true' || searchParams.get('mode') === 'inquiry';
      if (inquiry) setShowInquiryForm(true);
    }
...
const styles = {
  ...
  header: { backgroundColor: 'var(--primary-navy)', color: 'white', padding: 'clamp(25px, 5vw, 40px)', textAlign: 'center', borderBottom: '6px solid var(--primary-gold)' },
  paymentBox: { padding: 'clamp(25px, 4vw, 45px) clamp(15px, 4vw, 35px) 10px 40px', textAlign: 'center' },
  bankTitle: { color: 'var(--primary-navy)', fontSize: 'clamp(1.2rem, 4vw, 1.5rem)', marginBottom: '20px', fontWeight: '800' },
  bankWrapper: { backgroundColor: '#f8fafc', padding: 'clamp(20px, 4vw, 35px)', borderRadius: '24px', border: '2px dashed #cbd5e1' },
  ...
  accNumber: { color: 'var(--primary-gold)', fontSize: 'clamp(1.5rem, 6vw, 2.8rem)', letterSpacing: 'clamp(1px, 2vw, 4px)', fontWeight: 'bold', wordBreak: 'break-word' },
  accName: { fontSize: '1.1rem', fontWeight: '700', color: 'var(--primary-navy)', marginTop: '20px' },
  actionContainer: { padding: '0 clamp(15px, 4vw, 35px) clamp(25px, 5vw, 40px)', textAlign: 'center' },
  formOuter: { backgroundColor: '#f8fafc', padding: 'clamp(20px, 4vw, 35px)', borderRadius: '28px', border: '1px solid #e2e8f0' },
